...
func main() {
	...
	{
		service = tasks.NewFixedService()
		service = tasks.LoggingMiddleware(logger)(service)
		service = tasks.MetricsMiddleware(requestCount, requestLatency)(service)
	}

	// Endpoint domain.
	endpoints := tasks.MakeEndpoints(service, logger, tracer)
	//endpoints := user.MakeEndpoints(service, tracer)

	// HTTP router
	router := tasks.MakeHTTPHandler(ctx, endpoints, logger, tracer)
